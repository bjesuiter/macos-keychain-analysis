...
/**
 * Proof 20: security-created item with -T plus Team ID partition.
 *
 * Goal: create a fully artificial Always Allow-like state: /usr/bin/security
 * creates the item with `-T keychain-probe`, then `security
 * set-generic-password-partition-list` sets `apple-tool:,teamid:<TEAM>`.
 * This combines legacy trusted-app path trust with Team ID partition trust,
 * without clicking Always Allow on the read prompt.
 *
...
const service = "macos-keychain-analysis.proof-20.security-t-plus-teamid-partition";
const label = "macos-keychain-analysis proof 20 security T plus teamid partition";
...
const signingIdentity = process.env.PROOF20_CODESIGN_IDENTITY
...
async function askForLoginKeychainPassword(): Promise<string> {
  ...
  const script = [
    ...
    JSON.stringify("Proof 20 Keychain Password"),
...
async function main() {
  console.log("# Proof 20: security-created with -T plus teamid partition");
  ...
  console.log("The item is created with `security -T keychain-probe`, then gets `teamid:<TEAMID>` in its partition list.");
  console.log("After the artificial Always Allow-like state is set, record whether keychain-probe reads prompt. If a prompt appears, choose Allow Once, not Always Allow.");
  ...
  console.log("\n### Create item via /usr/bin/security with -T signed keychain-probe");
  await runSecurity(["add-generic-password", "-a", account, "-s", service, "-w", password, "-l", label, "-T", probePath]);

  console.log("\n### ACL list after -T create, before setting partition list");
  ...
  await readWithProbe("First password read via signed keychain-probe — should be silent if artificial Always Allow state is sufficient");
  ...
  console.log("\n## Cleanup command");
  console.log("bun run proof:20:cleanup");
}
