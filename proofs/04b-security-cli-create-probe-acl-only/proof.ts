...
/**
 * Proof 04b: create with security CLI, read ACLs only with keychain-probe.
 *
 * Goal: observe whether keychain-probe ACL inspection prompts when the item was
 * created by `/usr/bin/security`, without first reading the password value.
 */

const account = "macos-keychain-analysis";
const password = "disposable-proof-secret";
const service = "macos-keychain-analysis.proof-04b.security-create-probe-acl-only";
const label = "macos-keychain-analysis proof 04b security create probe acl only";
...
async function main() {
  ...
  console.log("\n## Case: security-create-probe-acl-only");
  console.log(`service: ${service}`);
  console.log("expected: security CLI creation should normally be silent. keychain-probe acl-list may prompt, but no password-value read is performed.");
  ...
  console.log("\n### Inspect ACL list only via keychain-probe");
  await runProbe(["acl-list", "--service", service, "--account", account]);

  console.log("\n## Cleanup command");
  console.log("bun run proof:04b:cleanup");
