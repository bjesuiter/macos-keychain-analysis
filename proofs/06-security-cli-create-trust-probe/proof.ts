...
/**
 * Proof 06: create with security CLI and explicitly trust keychain-probe.
 *
 * Goal: observe whether pre-authorizing keychain-probe via `security add-generic-password -T`
 * avoids GUI prompts for the first keychain-probe secret read.
 */

const account = "macos-keychain-analysis";
const password = "disposable-proof-secret";
const service = "macos-keychain-analysis.proof-06.security-create-trust-probe";
const label = "macos-keychain-analysis proof 06 security create trust probe";
...
async function printEnvironment() {
  ...
  await swVers.exited;
  console.log("# Proof 06: security CLI create and explicitly trust keychain-probe");
  console.log("\n## Environment");
...
async function main() {
  ...
  console.log("Record whether any command shows a GUI prompt. The expected result is no prompt for keychain-probe reads because it is trusted at creation time.");
  console.log(`Record observations in: ${observationPath}`);

  console.log("\n## Case: security-create-trust-probe");
  console.log(`service: ${service}`);
  console.log("expected: security CLI creation should be silent; ACL should include keychain-probe; keychain-probe reads should be silent.");
  ...
  console.log("\n## Cleanup command");
  console.log("bun run proof:06:cleanup");
