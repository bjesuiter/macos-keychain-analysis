...
/**
 * Proof 04a: create with security CLI, read with keychain-probe, no ACL read.
 *
 * Goal: observe whether keychain-probe can read a generic password created by
 * `/usr/bin/security`, without reading the item's ACLs afterward. This isolates the password-value read prompt.
 */

const account = "macos-keychain-analysis";
const password = "disposable-proof-secret";
const service = "macos-keychain-analysis.proof-04a.security-create-probe-read-no-acl";
const label = "macos-keychain-analysis proof 04a security create probe read no acl";
...
async function printEnvironment() {
  ...
  await swVers.exited;
  console.log("# Proof 04a: security CLI create, keychain-probe read, no ACL read");
  console.log("\n## Environment");
...
async function main() {
  ...
  console.log("expected: security CLI creation should normally be silent. keychain-probe read is expected to prompt once. No ACL read is performed, so there should be no second ACL-related prompt.");
  ...
  console.log("\n## Cleanup command");
  console.log("bun run proof:04a:cleanup");
